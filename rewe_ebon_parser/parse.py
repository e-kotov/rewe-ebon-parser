...
def parse_ebon(data_buffer: bytes) -> dict:
    try:
        ...
        print(f"Extracted Text: {data_text[:200]}...")  # Print a snippet of the extracted text for debugging

        lines = list(filter(None, map(str.strip, data_text.replace('  ', ' ').split('\n'))))
        print(f"Parsed Lines: {lines[:20]}...")  # Print the first 20 lines for debugging
        ...
        try:
            ...
            print(f"Market Address: {market_address.to_dict() if market_address else 'None'}")
